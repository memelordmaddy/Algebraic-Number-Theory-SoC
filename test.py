...
p_add = soc24mathlib.QuotitentPolynomialRing.Add(p1, p2)
assert p_add.element == [-2, 0, 6, 4] and p_add.pi_generator == [7, 0, 0,  3, 4]
p_sub = soc24mathlib.QuotitentPolynomialRing.Sub(p1, p2)
assert p_sub.element == [-4, -10, -8, -2] and p_sub.pi_generator == [7, 0, 0,  3, 4]
p_mul = soc24mathlib.QuotitentPolynomialRing.Mul(p1, p2)
assert p_mul.element == [11, 15, -68, -42] and p_mul.pi_generator == [7, 0, 0,  3, 4]
#p_div = soc24mathlib.QuotitentPolynomialRing.GCD(p1, p2)
#assert p_div.element == [1, 2, 1, 0] and p_div.pi_generator == [7, 0, 0,  3, 4]
'''
'''
test_var = False
try:
    p_inv = soc24mathlib.QuotitentPolynomialRing.Inv(p1)
except:
    test_var = True
assert test_var == True
p_inv = soc24mathlib.QuotitentPolynomialRing.Inv(soc24mathlib.QuotitentPolynomialRing([1, 0, 0, 0], [7, 0, 0,  3, 4]))
